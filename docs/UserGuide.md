...
* There are 5 colour options for `COLOUR_OF_TAGS_TO_ADD`: `RED`, `YELLOW`, `GREEN`, `BLUE` (default), and `PURPLE`.
  * You can either specify one colour to match 1-for-1 per tag in `a/` (separated by space, i.e `a/TAG1 TAG2 c/RED GREEN`, note the same number of tags and colours. See Ex 4)
  * Or specify one single colour to be applied to all tags (i.e `a/TAG1 TAG2 c/red`).
...
> If you add duplicate tag(s) (i.e. the contact already has a tag of that name, regardless of colour), an error message will pop up informing you of duplicates.
> If you add non-existent tag(s) (i.e. the contact does not have a tag with that name) will be silently ignored when deleting a mix of existing and non-existing tag(s).
